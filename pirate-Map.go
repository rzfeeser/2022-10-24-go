...
func main() {
   ...
   // set a key:value within our map
   myMap["keyExample"] = "valueExample"
   
   // some things in Go are "special" (however no overloading in func)
   // when you query a map the returns are "value" and "bool" (exist)
   //zach := myMap["keyExample"]        // capture just the value
   storedValue, existsOrNot := myMap["keyExample"] // capture value and bool 
   
   // display the results
   fmt.Println(storedValue)  // valueExample
   fmt.Println(existsOrNot)  // true
}
